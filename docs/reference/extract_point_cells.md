...
data for clustering at later points in the workflow. Output is in the
format of `H3` cells with a diameter of `hex_resolution`.
...
- interpolation:
  ...
  `add_var_raster` using nearest neighbour interpolation. Default =
  TRUE.

## Value

data.frame containing points from `habitat_raster` covering
`reef_polygon` for selected habitats in `habitat_categories`, converted
to H3 cell resolution, alongside extracted data from `add_var_raster`.
