...
# Pixel values that are used to extract target habitats from `habitat_raster`
...
) # Dataframe containing the H3 cell sizes for each resolution (from https://h3geo.org/docs/core-library/restable/#average-area-in-m2).

# Define the desired site size in terms of spatial area and the number of H3 cells per site
...
# Select target reef outline. For this example we use One Tree Island Reef from the Mackay - Capricorn region
...
# We must also attach a reef ID to the dataframe and remove any pixels with invalid depth data
