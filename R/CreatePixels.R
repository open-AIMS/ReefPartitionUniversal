...
CreatePixels <- function(
  ...
  intersects <- sf::st_intersects(
    sf::st_as_sfc(sf::st_bbox(geo_hab)),
    ROI,
    sparse = FALSE
  )
  ...
  if (!any(overlap)) {
    ...
    if (bbox_ROI["xmin"] < bbox_geo_hab["xmin"]) {
      bbox_ROI["xmin"] <- bbox_geo_hab["xmin"]
    }
    if (bbox_ROI["xmax"] > bbox_geo_hab["xmax"]) {
      bbox_ROI["xmax"] <- bbox_geo_hab["xmax"]
    }

    # Adjust ROI ymin and ymax if outside
    if (bbox_ROI["ymin"] < bbox_geo_hab["ymin"]) {
      bbox_ROI["ymin"] <- bbox_geo_hab["ymin"]
    }
    if (bbox_ROI["ymax"] > bbox_geo_hab["ymax"]) {
      bbox_ROI["ymax"] <- bbox_geo_hab["ymax"]
    }
  ...
  if (terra::ncell(geo_hab_cropped) == 0) {
    stop(
      "Cropping resulted in empty raster - check if ROI overlaps with valid raster data"
    )
  }
  ...
  if (length(hexid) > 1) {
    ...
    if (sf::st_crs(bathy_tif) != sf::st_crs(crop_extent)) {
      print(paste(
        "Transforming crop_extent from",
        sf::st_crs(crop_extent)$input,
        "to",
        sf::st_crs(bathy_tif)$input
      ))
      # Transform crop_extent to match bathy_tif's coordinate system
      crop_extent_wgs84 <- sf::st_transform(
        sf::st_as_sfc(crop_extent),
        sf::st_crs(bathy_tif)
      )
    ...
    hab_pts <- Pixels_clean %>%
      dplyr::mutate(
        id = hexid,
        area = h3::hex_area(res = resolution, unit = unit)
      ...
      dplyr::filter(
        !is.na(geomorph),
        if (!is.null(geozone_list)) geomorph %in% geozone_list else TRUE
